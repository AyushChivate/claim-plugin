...
public class UntrustCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        /* if the chunk trying to be un-trusted has not been previously claimed */
        ...
        /* make sure the player doesn't untrust themselves */
        if (player.getUniqueId().equals(otherPlayer.getUniqueId())) {
            player.sendMessage(StringUtils.colorString("&cYou cannot un-trust yourself!"));
            return false;
        }

        /* un-trust the specified player */
        claimedChunk.removeTrustedPlayer(otherPlayer);
        player.sendMessage(StringUtils.colorString("&aThis player has been un-trusted!"));
